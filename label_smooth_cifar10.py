...
for i in range(epoch):
    ...
    for ii in range(sample_per_class):
        ...
        for time in range(repetition):    
            ...
            test.reconstruct(iteration=iteration, cost_fn=cost_fn, lr=lr, optim_fn=optim_fn, magnify=1,label='hard',verble=verble,lr_decay=lr_decay,total_variation=total_v,keep=False,record_picking=True)
            ...
            rec_label[i,ii,time,2]=np.array(F.softmax(test.dummy_label.detach().cpu(), dim=-1))
            ...
            test.reconstruct(iteration=iteration, cost_fn=cost_fn, lr=lr, optim_fn=optim_fn, magnify=1,label='opt_dlg',verble=verble,lr_decay=lr_decay,total_variation=total_v,keep=False,record_picking=True)
